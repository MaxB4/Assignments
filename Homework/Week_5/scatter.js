...
window.onload = function () {
    ...
    Promise.all(requests).then(function (response) {
        ...
        yearsWomen = []
        datapointWomen = []
        lenWomenInScience = 47

        // read first file and fill lists
        var i;
        for (i = 0; i < lenWomenInScience; i++) {
            datapointWomen.push(womenInScience[i]["datapoint"])
            yearsWomen.push(womenInScience[i]["time"])
        }
        ...
        for (i = 0; i < (listMissingValues.length - 1); i++) {
            datapointWomen.splice(listMissingValues[i], 0, "None")
        }
        ...
        var i;
        for (i = 0; i < lenConsConf; i++) {
            yearsConsConf.push(consConf[i]["time"])
            country.push(consConf[i]["Country"])
            datapointConsConf.push(consConf[i]["datapoint"])
        }
        ...
        for (i = 0; i < countryList.length; i++) {
            for (j = 0; j < yearList.length; j++) {
                if (datapointWomen[k] !== "None") {
                    combinedList.push([datapointConsConf[k], datapointWomen[k], countryList[i], yearList[j]]);
                }
                k++;
            }
        }
        ...
        var xScale = d3.scaleLinear()
            .domain([d3.min(datapointConsConf, function (d) {
                return d;
            }), d3.max(datapointConsConf, function (d) {
                return d;
            })])
            .range([padding, w - padding * 2]);

        var yScale = d3.scaleLinear()
            .domain([d3.min(datapointWomen, function (d) {
                return d;
            }), d3.max(datapointWomen, function (d) {
                return d;
            })])
        ...
        svg.append("text")
            ...
            .text("Women in Science and Consumer Confidence in 6 Different Countries");
